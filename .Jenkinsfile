...
pipeline {
  ...
  stages {
    ...
    stage('Check Commit') {
      when {anyOf { allOf { branch "$ALLOWED_BRANCH" } ; allOf{expression{env.CHANGE_TARGET == "$ALLOWED_BRANCH"}} } }
      steps {
        script {
    ...
    stage('Run Test') {
      when { anyOf { allOf {branch "$ALLOWED_BRANCH"; expression{env.RUN_SONAR == '0'}} ; allOf {expression{env.CHANGE_TARGET == "$ALLOWED_BRANCH"}; expression{env.RUN_SONAR == '0'}}  } }
      agent {
        docker {
